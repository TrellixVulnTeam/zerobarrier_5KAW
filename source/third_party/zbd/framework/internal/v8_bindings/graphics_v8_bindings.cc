...
namespace GraphicsBindings {
  ...
  v8::Handle<v8::Value> thick_line(const v8::Arguments &args) {
    ...
    f32 thickness = 0.0f;
    verify_vm(ReadF32(args, 4, &thickness));

    Color color;
    verify_vm(ReadColor(args, 5, &color));

    Draw::ThickLine(start, end, thickness, color);
    return v8::Undefined();
  }
  ...
  v8::Handle<v8::Value> text(const v8::Arguments &args) {
    ...
    Draw::Text(text, start, font, color);
    return v8::Undefined();
  }
...
void AddGraphicsBindings(VM *vm) {
  ...
  graphicsTemplate->Set(v8::String::New("thick_line"), v8::FunctionTemplate::New(GraphicsBindings::thick_line));
  ...
  graphicsTemplate->Set(v8::String::New("text"), v8::FunctionTemplate::New(GraphicsBindings::text));
